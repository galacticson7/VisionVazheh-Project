...
from content.models import Word, Lesson # مدل Lesson برای کوئری جدید اضافه شد
...
# ... (تمام View های قبلی شما بدون تغییر باقی می‌مانند) ...
...
class ReviewListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        now = timezone.now()
        return UserWord.objects.filter(user=self.request.user, next_review_at__lte=now)
...
class CompletedLessonsView(views.APIView):
    ...
    def get(self, request, format=None):
        ...
        # ۱. تمام درس‌ها را به همراه تعداد کل لغاتشان می‌گیریم
        lessons = Lesson.objects.annotate(total_words=Count('words'))
        
        # ۲. لغاتی که کاربر به مرحله ۵ رسانده را فیلتر می‌کنیم
        mastered_words_by_lesson = UserWord.objects.filter(
            user=user, 
            learning_stage=5
        ).values(
            'word__lesson' # بر اساس درس گروه‌بندی کن
        ).annotate(
            mastered_count=Count('id') # تعداد لغات یادگرفته شده در هر درس را بشمار
        ).values('word__lesson', 'mastered_count') # نتیجه را برگردان

        # دیکشنری برای دسترسی آسان
        mastered_map = {item['word__lesson']: item['mastered_count'] for item in mastered_words_by_lesson}
        
        completed_lessons_ids = []
        for lesson in lessons:
            # ۳. مقایسه می‌کنیم: آیا تعداد کل لغات درس با تعداد لغات یادگرفته شده برابر است؟
...
class UpdateUserWordStatusView(views.APIView):
    ...
    def post(self, request, word_id, format=None):
        ...
        if action not in ['known', 'unknown']:
            return Response(
                {"error": "Action must be 'known' or 'unknown'."},
